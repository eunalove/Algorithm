...
public class Main {
		...
		for(int i=0; i<4; i++) {
			...
			if(cctvNum == 2) {
				...
				if(i == 0) {
					...
					lineMap(idx+7, 2, x, y);
					dfs(idx+1);
					lineMap2(idx+7, 2, x, y);
					lineMap2(idx+7, 0, x, y);
				...
				if(i == 1) {
					...
					lineMap(idx+7, 3, x, y);
					dfs(idx+1);
					lineMap2(idx+7, 3, x, y);
					lineMap2(idx+7, 1, x, y);
				}
			}
	...
	private static void lineMap2(int idx, int dir, int x, int y) { //벽에 부딪히거나 맵의 끝에 닿을 때까지 dir방향으로 cctvNum을 표시한다.
